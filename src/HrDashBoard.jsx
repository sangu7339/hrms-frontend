...
const validateSearch = () => {
  const errors = {};
  const { username, firstName, departmentName, designationName, startDate, endDate } = searchParams;

  if (username && !/^[a-zA-Z]{4}[0-9]{4}$/.test(username))
    errors.username = "Format: VPPL001";

  if (firstName && !/^[A-Za-z]{2,}$/.test(firstName))
    errors.firstName = "Only letters (min 2)";

  if (departmentName && !/^[A-Za-z ]{2,50}$/.test(departmentName))
    errors.departmentName = "Only alphabets";

  if (designationName && !/^[A-Za-z ]{2,50}$/.test(designationName))
    errors.designationName = "Only alphabets";
  ...
  if (!username && !firstName && !departmentName && !designationName && !startDate && !endDate)
    errors.general = "Enter at least one field";
...
case "holidaycalendar":
  ...
  value={searchParams.username}
  onChange={(e) => {
    let val = e.target.value.toUpperCase().replace(/[^A-Z0-9]/g,"").slice(0,8);
    setSearchParams({ ...searchParams, username: val });
    
    // Validate VPPL + numeric digits
    if (val.length > 0) {
      if (!val.startsWith("VPPL")) {
        setSearchErrors(prev => ({ ...prev, username: "Must start with VPPL" }));
      } else {
        const numPart = val.slice(4);
        if (numPart.length > 0 && !/^\d{3,4}$/.test(numPart)) {
          if (!/^\d+$/.test(numPart) && numPart.length > 0) {
            setSearchErrors(prev => ({ ...prev, username: "Format: VPPL001 or VPPL0001 (numbers only)" }));
          } else if (numPart.length > 0 && numPart.length < 3) {
            setSearchErrors(prev => ({ ...prev, username: "Need 3 or 4 digits after VPPL" }));
          } else if (numPart.length > 4) {
            setSearchErrors(prev => ({ ...prev, username: "Maximum 4 digits after VPPL" }));
          }
        } else if (/^\d{3,4}$/.test(numPart)) {
          setSearchErrors(prev => { const newErrors = { ...prev }; delete newErrors.username; return newErrors; });
        }
      }
    } else {
      setSearchErrors(prev => { const newErrors = { ...prev }; delete newErrors.username; return newErrors; });
...
/>
{searchErrors.username && (
  <span className="error-text">{searchErrors.username}</span>
)}
                  </div>
  ...
  onChange={(e) => {
    ...
    let cleanVal = val.replace(/[^A-Za-z]/g,"");
    
    setSearchParams({ ...searchParams, firstName: cleanVal });
    
    if (hasNumbers) {
      setSearchErrors(prev => ({ ...prev, firstName: "Only letters (no numbers)" }));
    } else if (cleanVal.length > 0 && cleanVal.length < 2) {
      setSearchErrors(prev => ({ ...prev, firstName: "Only letters (min 2)" }));
    } else {
      setSearchErrors(prev => { const newErrors = { ...prev }; delete newErrors.firstName; return newErrors; });
    }
  }}
/>
{searchErrors.firstName && (
  <span className="error-text">{searchErrors.firstName}</span>
)}
                  </div>
  ...
  onChange={(e) => {
    ...
    setSearchParams({ ...searchParams, departmentName: cleanVal });
    
    if (hasNumbers) {
      setSearchErrors(prev => ({ ...prev, departmentName: "Only letters (no numbers)" }));
    } else if (cleanVal.length > 0 && cleanVal.length < 2) {
      setSearchErrors(prev => ({ ...prev, departmentName: "Only alphabets" }));
    } else {
      setSearchErrors(prev => { const newErrors = { ...prev }; delete newErrors.departmentName; return newErrors; });
    }
  }}
/>
{searchErrors.departmentName && (
  <span className="error-text">{searchErrors.departmentName}</span>
)}
                  </div>
  ...
  onChange={(e) => {
    ...
    setSearchParams({ ...searchParams, designationName: cleanVal });
    
    if (hasNumbers) {
      setSearchErrors(prev => ({ ...prev, designationName: "Only letters (no numbers)" }));
    } else if (cleanVal.length > 0 && cleanVal.length < 2) {
      setSearchErrors(prev => ({ ...prev, designationName: "Only alphabets" }));
    } else {
      setSearchErrors(prev => { const newErrors = { ...prev }; delete newErrors.designationName; return newErrors; });
    }
  }}
/>
{searchErrors.designationName && (
  <span className="error-text">{searchErrors.designationName}</span>
)}
                  </div>
                </div>
...
max={new Date().toISOString().split("T")[0]}
                    ...
                    {searchErrors.date && (
                      <span className="error-text">{searchErrors.date}</span>
                    )}
                  </div>
                </div>

                {searchErrors.general && (
                  <div className="form-error-box">
                    <span className="error-text">{searchErrors.general}</span>
                  </div>
                )}
